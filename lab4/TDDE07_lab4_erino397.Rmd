...
### (d)
After having determined the predictive distribution of the sample $\hat{y}$ as $p(\hat{y}|\lambda)\sim{Poisson(\lambda)}$, where $\lambda=e^{\hat{X}\beta}$ using the betas draws during the simulation in (c) and $\hat{X}$ as the sample to predict, I got the distribution seen in Figure 2. The probability that the sample has zero bidders by this distribution was $p(\hat{y}=0|\lambda)\approx0.36$.
